...
def convertUniWhoosh(word):
	return  unicode(unidecode(unicode(word,'utf-8')),'utf-8')
def createFoodWhoosh(id, food_item_info,writer):
	#size of this list is 25
	type = convertUniWhoosh(food_item_info[1])
	food = convertUniWhoosh(food_item_info[2])
	detail = convertUniWhoosh(food_item_info[3])
	source = convertUniWhoosh(food_item_info[4])
	tag = convertUniWhoosh(food_item_info[2]+" "+food_item_info[3])
...
def addAllFood(session,dict,ix):
	j_result = -23
	dict = createDict()
	while j_result <= 826:
		print j_result
...
# 				print eachBasicInfo[k]
			...
			foodItem = createFood(eachBasicInfo)
 			session.add(foodItem)
 			session.commit()
			foodId= foodItem.id
			#Whoosh
 			createFoodWhoosh(foodId, eachBasicInfo,writer)
 		writer.commit()
			###
